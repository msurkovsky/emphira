...
int main (int argc, char **argv) {
  ...
  if (rank == 0) {
    ...
    for (int p = 1; p < size; p++) {
      ...
      data_a = new char[sub_a.get_byte_size()];
      sub_a.serialize(data_a);
      // TODO: may have a problem with standard send (blocking one)
      MPI_Send(data_a, sub_a.get_byte_size(), MPI_BYTE, p, A, MPI_COMM_WORLD);
      ...
      data_b = new char[sub_b.get_byte_size()];
      sub_b.serialize(data_b);
      MPI_Send(data_b, sub_b.get_byte_size(), MPI_BYTE, p, B, MPI_COMM_WORLD);
    }
    ...
    data_a = new char[msg_size];
    MPI_Recv(data_a, msg_size, MPI_BYTE, 0, A, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    sub_a.deserialize(data_a);


    MPI_Probe(0, B, MPI_COMM_WORLD, &status);
    MPI_Get_count(&status, MPI_BYTE, &msg_size);

    data_b = new char[msg_size];
    MPI_Recv(data_b, msg_size, MPI_BYTE, 0, B, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    sub_b.deserialize(data_b);
  }
  ...
  for (int i = 1; i < n; i++) { // from 1 because the first round is done
    ...
    MPI_Irecv(data_b, // use data_b for receive in order not to rewrite data_a buffer
              sub_a.get_byte_size(),
              MPI_BYTE,
              MPI_ANY_SOURCE,
              A,
  ...
  if (rank == 0) {
    ...
    for (int k = 1; k < size; k++) {
      MPI_Recv(data_a, sub_c.get_byte_size(),
               MPI_BYTE,
               MPI_ANY_SOURCE,
               C,
               MPI_COMM_WORLD,
    ...
    // NOTE: this is also something I can check
  } else {
    sub_c.serialize(data_a);
    MPI_Send(data_a,
             sub_c.get_byte_size(),
             MPI_BYTE,
             0,
